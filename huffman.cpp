...
string huffman::decToBin(int inNum) {
    ...
    for (int i = temp.length() - 1; i >= 0; i--) {     // reversing it
        res += temp[i];
    }
    return res;
}
...
void huffman::getTree() {
    ...
    for(int i = 0; i < size; i++) 
    {
        ...
        inFile.read(&aCode, 1);   // reading Character
        inFile.read(reinterpret_cast<char*>(hCodeC), 16);  // reading remaining 16 decimals
